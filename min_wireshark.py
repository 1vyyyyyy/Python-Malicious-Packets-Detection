...
# 网卡的名称,这里设置为虚拟机的网卡
capt_interface='VMware Virtual Ethernet Adapter for VMnet8'
...
#     packet_time = stamp_2_time(packet.time)
#     src = packet[IP].src
#     dst = packet[IP].dst
#     proto = 'http'
#     length = len(packet)
...
# 生产者函数
def packet_producer():
    sniff(prn = lambda pkt: packet_consumer(pkt), stop_filter = lambda pkt: stop_sniff.is_set(),
          filter = fitler_entry.get(), iface = capt_interface)
...
def packet_consumer(pkt):
    ...
    if not pause_sniff.is_set():
        ...
        packet_time = stamp_2_time(pkt.time)
        # 判断数据包的协议类型
...
open_button = Button(toolbar, width = 8, text = "打开pcap", command = read_pcap)
save_button = Button(toolbar, width = 8, text = "保存数据", command = save_pcap_2_file)
...
packet_list_tree.bind('<<TreeviewSelect>>', click_on_list_tree)
